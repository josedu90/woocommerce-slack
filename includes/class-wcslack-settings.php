...
if ( ! class_exists( 'WC_Slack_Settings' ) ) {
	...
	class WC_Slack_Settings extends WC_Integration {
		...
		public function init_form_fields() {
			...
			$this->form_fields = array(
				...
				'emoji' => array(
					...
					'description'       => sprintf( __( 'Enter the code for an emoji you want to include with each message by default. You can find all codes on the %s or you can even upload your own icon as an emoji in %s', 'woocommerce-slack' ), '<a href="http://www.emoji-cheat-sheet.com/" target="_blank">Emoji Cheat Sheet</a>', '<a href="https://slack.com/customize/emoji" target="_blank">Slack</a>!' ),
					'default'           => '',
				),
				'name' => array(
				...
				'debug' => array(
					...
					'type'              => 'checkbox',
					'label'             => __( 'Enable Debug Mode', 'woocommerce-slack' ),
					'default'           => 'no',
					'desc_tip'          => true,
					'description'       => __( 'Find the logs under WooCommerce > System Status > Logs', 'woocommerce-slack' ),
				),
				...
				'notif-post-new-message' => array(
					...
					'description'       => '<strong>Optional:</strong> You can write a custom notification message. <a href="http://docs.woothemes.com/document/woocommerce-slack#section-3">Valid Slack Markup Allowed</a><br />
		...
		public function scripts() {
			...
			wp_register_script( 'wcslack-admin-js', plugins_url( 'assets/js/wcslack-admin.js', dirname( __FILE__ ) ), array( 'jquery') );
		...
		public function available_channels() {
			...
			if ( $api_key ) {
				...
				if ( false === ( $all_channels = get_transient( 'wcslack_all_channels' ) ) ) {
					$all_channels = $wcslack_api->all_channels( $api_key );
					set_transient( 'wcslack_all_channels', $all_channels, 86400 );
				}

				if ( false === ( $all_groups = get_transient( 'wcslack_all_groups' ) ) ) {
					$all_groups = $wcslack_api->all_groups( $api_key );
